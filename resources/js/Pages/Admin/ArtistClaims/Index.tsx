...
interface Claim {
    ...
    message?: string | null;
    artist: { name: string; slug: string; user_id?: number | null };
...
export default function ArtistClaimsIndex({ claims }: Readonly<Props>) {
    return (
        <AdminLayout>
            <SeoHead title="Sanatçı Sahiplenme Talepleri" description="Sanatçı sahiplenme başvuruları." noindex />
            <div className="space-y-6">
                <AdminPageHeader title="Sanatçı sahiplenme talepleri" description="Başvuruları inceleyip onaylayın veya reddedin." />
                ...
                <ul className="space-y-3" role="list">
                    {claims.data.map((claim) => (
                        ...
                        >
                            <p className="font-semibold text-zinc-900 dark:text-white">{claim.artist.name}</p>
                                ...
                                {claim.status === 'pending' && (
                                    <>
                                        <button
                                            ...
                                            onClick={() => router.post(route('admin.artist-claims.approve', claim.id))}
                                        >
                                            Onayla
                                        </button>
                                        <button
                                            ...
                                            onClick={() => router.post(route('admin.artist-claims.reject', claim.id))}
                                        >
                                            Reddet
                                        </button>
                                    </>
